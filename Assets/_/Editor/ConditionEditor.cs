...
namespace Prototype.Editor
{
    ...
    [CustomEditor(typeof(Trigger2DCondition))]
    public class Trigger2DConditionEditor : ConditionEditor
    {
        protected override VisualElement CreateInspectorGUI_If()
        {
            var element = new VisualElement();
            ...
            {
                label = "",
                bindingPath = "stayTrue"
            };
            field.BindProperty(serializedObject);

            var group = new VisualElement { style = { flexDirection = FlexDirection.Row } };
            group.style.SetPadding(2, 0);
            group.Add(field);
            group.Add(new Label("  Stay True"));
            element.Add(group);
    ...
    [CustomEditor(typeof(OverlapPoint2DCondition))]
    public class OverlapPoint2DConditionEditor : ConditionEditor
    {
        protected override VisualElement CreateInspectorGUI_If()
        {
            var element = new VisualElement();
    ...
    [CustomEditor(typeof(ObjectActivityCondition))]
    public class ObjectActivityConditionEditor : ConditionEditor
    {
        protected override VisualElement CreateInspectorGUI_If()
        {
            var element = new VisualElement();
    ...
    [CustomEditor(typeof(IsButtonDownCondition))]
    public class IsButtonDownConditionEditor : ConditionEditor
    {
        protected override VisualElement CreateInspectorGUI_If()
        {
            var element = new VisualElement();
    ...
    [CustomEditor(typeof(IsMouseDownCondition))]
    public class IsMouseDownConditionEditor : ConditionEditor
    {
        protected override VisualElement CreateInspectorGUI_If()
        {
            var element = new VisualElement();

            foreach (var mouseButton in new[] { "Left", "Right", "Middle" })
            {
                var field = new PropertyField
                {
                    label = $"Check {mouseButton} Mouse Button",
                    bindingPath = $"{mouseButton.ToLower()}MouseButton"
                };
